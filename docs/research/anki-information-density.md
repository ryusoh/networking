...
D_{\text{lex}} &= \operatorname{MTLD}(\operatorname{tokens}(x), \text{threshold}=0.72, \text{bidirectional}) \quad &&\text{(lexical diversity)} \\
D_{\text{concept}} &= \frac{|\text{distinct technical tokens}|}{|\operatorname{tokens}(x)|} \times 100 \quad &&\text{(technical-form proxy)} \\
