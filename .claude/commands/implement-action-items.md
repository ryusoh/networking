...
Execute the work orders in **$ARGUMENTS** (or the doc from the conversation)
...
1. **Read the tags.** `[skip]` → skip the item entirely and record it as
   ...
   but flag "needs human/hardware review" in your final report.
...
3. **Make exactly the stated change.** Nothing more: no drive-by edits, no
   refactors, no comment rewording, no touching generated output dirs
   (`coverage/`, `adblock/dist/`, `nas_proxy/out/`). Keep each item within a
   single subproject (AGENTS.md non-negotiable #2).
