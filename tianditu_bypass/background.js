...
function updateProxySettings(server) {
  ...
  const failedHostsList = Array.from(failedHosts).map(h => `"${h}"`).join(",");
  ...
  chrome.proxy.settings.set({ value: config, scope: 'regular' }, () => {
    console.log(`Tianditu Hybrid config updated. Current Proxy: ${server}. Blocked Hosts: ${failedHosts.size}`);
  });
}
...
chrome.webRequest.onErrorOccurred.addListener(
  (details) => {
    const host = new URL(details.url).hostname;
    if (!failedHosts.has(host)) {
      console.warn(`Connection failure (${details.error}) for ${host}. Switching to proxy fallback.`);
      failedHosts.add(host);
      updateProxySettings(currentProxyServer);
    }
  },
...
async function fetchFromSource(source) {
  try {
    ...
    if (result && result.proxies && result.proxies.length > 0) {
      ...
      console.log(`[BENCHMARK] ${source.name}: Found ${result.proxies.length} proxies in ${fetchTime}ms. Avg Latency: ${avg}ms. Fastest: ${min}ms.`);
    } else {
    ...
    console.error(`[BENCHMARK] ${source.name} failed: ${e.message}. Check host permissions in manifest.json if TypeError.`);
    return [];
  }
}
