...
- **Hard bans (non-negotiable #6):** no new dependencies; no edits to
  `package.json`, `jsconfig.json`, `pyproject.toml`, `eslint.config.cjs`,
  `eslint-suppressions.json`, or any `Makefile`; no architectural changes;
  no breaking changes; never trade readability for a micro-optimization; never
