...
model can execute them mechanically. Input doc: **{{args}}** (or the doc from
the conversation).
...
1. **Preamble — rules for the implementer**, e.g.: one work order per change;
   ...
   and lint commands to run after editing (`npx prettier --write <file>`,
   `npx eslint <file>`); never hand-edit generated output (`coverage/`,
   `adblock/dist/`, `nas_proxy/out/`) or commit command logs.
2. **Numbered work orders**, ranked by expected impact, each tagged:
   - `[trivial]` — markup/config/deletion only
   - `[low]` — small logic change
   - `[visual]` — rendered extension UI or live network behaviour; a human must
     review afterwards
...
Run the repo's formatter on the doc (`npx prettier --write <doc>`) and tell the
user where the work orders live and which items are tagged `[skip]`.
