...
Arista EOS's SysDB design: the disk-backed state ledger (`.agents/state/` or
governing findings doc) is the single source of truth; worker agents are
ephemeral, stateless transforms ($O(1)$ context) reading only their active gate
slice. At each transaction boundary (after commits or gate checks) and on
session resumption, follow the skill's `## Resume protocol`: re-anchor working
